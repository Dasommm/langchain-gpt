#### fastAPI를 사용하여 openapi를 생성한 후,
## cloudflared로 배포 한 후 스키마를 사용하여 gpt가 내 함수를 사용할 수 있도록 할 수 있다.
## 그러나 현재는 이런 방식으로 실행되지 않고, MCP로 연결을 해야한다.
## MCP 서버를 생성해야하고 아래의 방식으로는 해결되지 않고, 보통 node.js + javascript로 mcp 서버를 만든 후
## gpt에 이 mcp 서버를 연결하여 사용한다.
